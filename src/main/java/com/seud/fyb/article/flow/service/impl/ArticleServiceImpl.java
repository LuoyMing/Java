...
public abstract class ArticleServiceImpl implements IArticleBusiService{
	
	protected FlowItemsTypeEnum flowItemsTypeEnum;
	...
	public Map<String,Object> start(Map<String,Object> params){
		...
		try{
			...
			logger.error("ArticleContentProduceBusiServiceImpl,start执行失败！");
	...
	public Map<String,Object> audit(Map<String,Object> params){
		...
		boolean isAgree=Boolean.parseBoolean(String.valueOf(params.get("isAgree")));
			...
			if(!flowItemsRecordService.updateFlowAduitItmesInfo(audititemsId, isAgree, flowItemsTypeEnum.getEffectiveType())){
				logger.error("ArticleRemoveServiceImpl,audit执行失败！");
				throw new RuntimeException("执行失败");
			}else{
				if(isAgree&&!effective(params)){
	...
	protected void setFlowItemsTypeEnum(FlowItemsTypeEnum flowItemsTypeEnum) {
		this.flowItemsTypeEnum = flowItemsTypeEnum;
	}
